...
public class Episode implements Sequenceable<Episode>, Cloneable {
	...
	/**
	 * Creates a new Episode with some cloned elements
	 * This is used to create a new Episode for the TV Show
	 */
	@Override
	public Episode clone() {
		return new Episode(aPath, aTVShow, aTitle, aEpisodeNumber);
	}
	...
	public String getTVShow() {
		return aTVShow.getTitle();
	}
	
	
	/**
	 * Updates the name given to the episode
	 *
	 * @param pTitle
	 *            the name of the individual episode
	 * @pre pTitle!=null
	 */
	public void setName(String pTitle) {
		assert pTitle != null;
		aTitle = pTitle;
	}
	...
	/**
	 * Updates the file path of the episode
	 *
	 * @param pPath
	 *            the file path of the individual episode
	 * @pre pPath!=null
	 * @throws IllegalArgumentException
	 *             if the path doesn't point to a file (e.g., it denotes a directory)
	 */
	public void setFilePath(File pPath) {
		assert pPath != null;
		if (pPath.exists() && !pPath.isFile()) {
			throw new IllegalArgumentException("The path should point to a file.");
		}
	 ...
	 * Useful when adding a new episode to a tv show
	 *
	 * @param pEpisodeNumber
	 * 			The integer used to denote the episode number
	 * @pre aEpisode > 0
	 * 			The episode number should always be valid
	 * 			Episodes start at 1
	 */
	public void setEpisodeNumber(int pEpisodeNumber) {
		assert aEpisodeNumber > 0;
		aEpisodeNumber = pEpisodeNumber;
	}
}
