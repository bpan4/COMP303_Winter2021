...
public class Movie implements Sequenceable<Movie> {
	...
	private static Map<String, Movie> moviesCache = new HashMap<>();
	...
	/**
	 * Generates a Movie object based on the title provided OR returns a Movie object that was already created. 
	 * Callers must also provide required metadata about the movie.
	 *
	 * @param pPath
	 *            location of the movie on the file system.
	 * @param pTitle
	...
	public static Movie getMovie(File pPath, String pTitle, Language pLanguage, String pStudio) {
		assert pTitle!=null;
	    Movie newMovie = (Movie) moviesCache.get(pTitle);
	    if (newMovie == null) {
	    	newMovie = new Movie(pPath, pTitle, pLanguage, pStudio);
	    	moviesCache.put(pTitle, newMovie);
	    }
	    return newMovie;
	}
